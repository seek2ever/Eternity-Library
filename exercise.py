...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.fetch_and_display_books()  # 在窗口加载时显示数据库中的书籍
    ...
    def scan_books(self):
        ...
        selected_files = scan_dialog.select_directory()  # 假设返回已选择的文件路径列表

        # 假设 DatabaseManager 类有 add_books 方法用于添加书籍信息到数据库
        book_db = DatabaseManager()
        for file_path in selected_files:
            book_info = extract_book_info_from_file(file_path)  # 实现该函数以从文件提取书籍信息
            book_db.add_books(book_info)
    ...
    def fetch_and_display_books(self):
        """从数据库中获取书籍信息并显示在列表框中"""
        book_db = DatabaseManager()
        books = book_db.get_all_books()  # 假设 get_all_books 方法返回所有书籍信息列表

        self.listWidget.clear()  # 清空现有列表项
        for book in books:
            item_text = f"{book['name']} - {book['author']}"  # 根据实际书籍数据结构调整
